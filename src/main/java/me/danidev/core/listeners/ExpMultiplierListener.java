...
public class ExpMultiplierListener implements Listener {
	...
	public ExpMultiplierListener(Main plugin) {
	...
	public void onEntityDeath(EntityDeathEvent event) {
		...
		if (killer != null && amount > 0.0 && (stack = killer.getItemInHand()) != null
				&& stack.getType() != Material.AIR
				&& (enchantmentLevel = stack.getEnchantmentLevel(Enchantment.LOOT_BONUS_MOBS)) > 0L) {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (stack != null && stack.getType() != Material.AIR && amount > 0.0
				&& (enchantmentLevel = stack.getEnchantmentLevel(Enchantment.LOOT_BONUS_BLOCKS)) > 0) {
	...
	public void onPlayerFish(PlayerFishEvent event) {
		...
		if (amount > 0.0) {
			...
			if (projectileSource instanceof Player
					&& (enchantmentLevel = (((Player) projectileSource).getItemInHand())
							.getEnchantmentLevel(Enchantment.LUCK)) > 0L) {
