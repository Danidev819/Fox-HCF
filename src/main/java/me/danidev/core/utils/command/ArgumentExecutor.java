...
public abstract class ArgumentExecutor implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (args.length < 1) {
			...
			sender.sendMessage(ChatColor.DARK_AQUA + ChatColor.BOLD.toString() + WordUtils.capitalizeFully(label) + " Help" + ChatColor.GRAY + " (Page 1 out of 1)");
			sender.sendMessage(" ");
			for (CommandArgument argument : this.arguments) {
				...
				new Text(ChatColor.AQUA + argument.getUsage(label) + ChatColor.GRAY + " - " + ChatColor.WHITE
