...
public enum VisualType {
	...
	CLAIM_MAP() {
		...
		{
			this.blockFiller = new BlockFiller() {
				private final Material[] types = { Material.SNOW_BLOCK, Material.SANDSTONE, Material.FURNACE,
				...
				VisualBlockData generate(final Player player, final Location location) {
					...
					final Faction faction = Main.get().getFactionManager().getFactionAt(location);
					return new VisualBlockData(Material.STAINED_GLASS,
							((faction != null) ? faction.getRelation((CommandSender) player) : Relation.ENEMY)
									.toDyeColour().getData());
				}
