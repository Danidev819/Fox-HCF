...
public class LeaderboardKillsButton extends Button {

    private final Comparator<PlayerFaction> KILLS_COMPARATOR = Comparator.comparingInt(PlayerFaction::getKills);
    ...
    public ItemStack getButtonItem(Player player) {
        ...
        List<PlayerFaction> data = Main.get().getFactionManager().getFactions().stream()
                ...
                .filter(x -> x.getKills() > 0).distinct()
                .sorted(KILLS_COMPARATOR)
        ...
        if (data.isEmpty()) {
            ...
            for (int i = 0; i < 10 && i < data.size(); ++i) {
                ...
                lore.add("&7" + (i + 1) + ") &b" + next.getName() + ": &f" + next.getPoints());
            }
        }
