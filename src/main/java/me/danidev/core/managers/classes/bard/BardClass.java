package me.danidev.core.managers.classes.bard;
...
public class BardClass extends PvPClass implements Listener {
	...
	public BardClass(Main plugin) {
		...
		this.bardEffects.put(Material.SPIDER_EYE,
				new BardEffect(45, new PotionEffect(PotionEffectType.WITHER, 100, 1), null));
	...
	public boolean onEquip(Player player) {
		...
		if (BardClass.this.plugin.getTimerManager().protectionTimer.getRemaining(player.getUniqueId()) > 0L) {
	...
	private boolean canUseBardEffect(Player player, BardData bardData, BardEffect bardEffect, boolean sendFeedback) {
		...
		if (remaining > 0L) {
			errorFeedback = ChatColor.RED + "You still have a cooldown on this " + ChatColor.GREEN + ChatColor.BOLD
					+ "Bard" + ChatColor.RED + " buff for another " + DurationFormatter.getRemaining(remaining, true, false)
					+ ChatColor.RED + '.';
		}
	...
	private static long BUFF_COOLDOWN_MILLIS = TimeUnit.SECONDS.toMillis(8L);
	private Map<UUID, BardData> bardDataMap;
	private Map<Material, BardEffect> bardEffects;
	private BardRestorer bardRestorer;
	private Main plugin;
