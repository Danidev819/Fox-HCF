...
import me.danidev.core.utils.visualise.VisualBlock;
...
public class ClaimWandListener implements Listener {
    ...
    public ClaimWandListener(Main plugin) {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (action == Action.RIGHT_CLICK_AIR) {
            Main.get().getClaimHandler().clearClaimSelection(player);
            player.setItemInHand(new ItemStack(Material.AIR, 1));
            player.sendMessage(ChatColor.RED + "You have cleared your claim selection.");
            return;
        }
        PlayerFaction playerFaction = Main.get().getFactionManager().getPlayerFaction(uuid);
        if ((action != Action.LEFT_CLICK_AIR && action != Action.LEFT_CLICK_BLOCK) || !player.isSneaking()) {
            if (action == Action.LEFT_CLICK_BLOCK || action == Action.RIGHT_CLICK_BLOCK) {
                ...
                if (Main.get().getClaimHandler().canClaimHere(player, blockLocation)) {
                    ClaimSelection revert = new ClaimSelection(blockLocation.getWorld());
                    ClaimSelection claimSelection = (ClaimSelection) Main.get().getClaimHandler().claimSelectionMap.putIfAbsent(uuid, revert);
                    ...
                    if (oldPosition != null) {
                        ...
                        Main.get().getVisualiseHandler().clearVisualBlocks(player, VisualType.CREATE_CLAIM_SELECTION, (Predicate<VisualBlock>)new Predicate<VisualBlock>() {
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            Main.get().getVisualiseHandler().generate(player, locations, VisualType.CREATE_CLAIM_SELECTION, true);
                        }
                    }.runTask(Main.get());
                }
            }
        ...
        ClaimSelection claimSelection2 = (ClaimSelection) Main.get().getClaimHandler().claimSelectionMap.get(uuid);
        ...
        if (Main.get().getClaimHandler().tryPurchasing(player, claimSelection2.toClaim(playerFaction))) {
            Main.get().getClaimHandler().clearClaimSelection(player);
            player.setItemInHand(new ItemStack(Material.AIR, 1));
        }
    }
    ...
    public void onPlayerDrop(PlayerDropItemEvent event) {
        ...
        if (this.isClaimingWand(item.getItemStack())) {
            item.remove();
            Main.get().getClaimHandler().clearClaimSelection(event.getPlayer());
        }
    }
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        if (event.getDrops().remove(ClaimHandler.CLAIM_WAND)) {
            Main.get().getClaimHandler().clearClaimSelection(event.getEntity());
        }
    }
